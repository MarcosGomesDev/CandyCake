...
import { UserProps } from '../utils/storage';

const URL = 'http://192.168.1.3:3003'
...
async function registerUser(name, lastname, email, password) {
    const response = await axios.post(`${URL}/sign-up/user`, { name, lastname, email, password })
...
async function verifyUserTokenPasswordReset(token: string) {
    const data = {
        token: token
    }
...
// ============================= ROTAS DE PRODUTOS =============================
async function getProducts({_id, token}: any) {
